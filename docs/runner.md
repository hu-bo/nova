...
避免已完成的前台命令因后台子进程持有管道而被误报超时。输出排空有 1 秒上限，防止无法终止的
后代进程让已结束的 execution 永久等待。

`timeout_ms` 从获得运行额度、开始创建进程时计时，不包含排队和网络传输时间。
超时或取消后，进程退出与输出排空共用 1 秒收尾上限；到期仍未退出或 EOF 时记录
`execution_cleanup_timeout` 并结束执行流，不能无限等待 kill 的结果。进程已正常退出后，
输出排空不会将结果改成 `timed_out`。超时日志同时记录实际 `duration_ms`，用于区分
配置时限与实际执行耗时；它不代表整个 tool 调用（含排队、传输、外层收尾）的耗时。
...
Execution driver 在独立任务中运行，有界事件通道负责向 transport 交付；等待通道空间时仍监听
进程退出、超时和取消。收尾到期后放弃未交付的输出并标记截断，先释放运行额度，再等待交付
`Finished`。网络不可写会延迟事件送达，但不能延迟本地终止进程。
