...
图片附件按结构化 `images` 发送，文字为空时也允许发送图片；普通文件仍作为附件链接。重试与本地排队保留图片对象 key，重试复用上传对象。发送前检查模型图片能力、数量、格式和大小；最终以服务端验证为准。
...
首次发送成功后，先取消可能返回旧列表的在途查询，把创建 API 返回的完整会话写入对应列表缓存，
再替换 URL。导航不等待列表重新获取；后台刷新失败不应让已创建的会话短暂显示“不存在”。
实时消息已到达时，历史查询在后台补齐消息，不替换整个聊天界面；历史合并保留实时审批与运行状态。
首条消息发送成功后立即刷新正在展示的会话列表，使用服务端由首问生成的标题；普通 Chat 与 Project Chat
遵循同一规则，导航不等待刷新完成。已有内存消息时不重新应用 Query 中缓存的旧历史快照。
合并历史时按消息 id 去重，以共有消息为位置锚点保留本地消息的先后顺序；没有共有消息时按创建时间合并，
不能把快照缺少的首条用户消息直接追加到历史末尾。

首次创建返回后，乐观消息与 SSE 统一写入真实会话 id，临时页立即订阅该状态。首次发送成功替换 URL
时保留 Chat 组件实例，避免清空输入区、滚动位置和发送反馈。已有内存消息的会话不因这次切换或返回页面
重新请求历史；刷新页面后没有内存消息时才加载历史，显式重试与 RESYNC 仍可重新同步。
...
待处理的 Decision 由 `Chat` 悬浮在消息区底部（`chat-ui.md` §6），不与 TODO 和 Composer 争竖向空间，
窄屏上允许、拒绝和取消按钮才不会顶出裁剪视口。
...
`context.updated` 覆盖。自动或手动压缩成功后，SSE `context.compacted` 明确展示压缩原因；摘要失败而
回退到省略中段时也会如实告知，避免上下文百分比下降却没有解释。Composer 底部与模型选择并列展示
...
| 临时会话首次发送               | 创建 conversation，并把用户消息乐观写入真实 id 的 reducer，临时页订阅同一状态 | 用真实 id 建流后 POST 消息；成功后保留 Chat 实例替换 URL 并标记会话列表 stale；创建或发送失败时保留草稿供重试 |
...
服务端运行状态是 loading 的依据；暂停显示原因与继续/停止操作。重同步允许服务端
终态覆盖本地 streaming 消息，保留已展示内容但结束未完成工具。SSE 自动重连之外
周期读取运行状态，避免遗漏 run.end 后卡住。
