...
export interface Message { id: string; role: "user" | "assistant"; blocks: Block[]; createdAt: number }
export type Block = { type: "text"; text: string } | { type: "thinking"; text: string; signature?: string; data?: { format: "deepseek" } | { format: "minimax"; details: unknown[] } } | { type: "tool_call"; callId: string; name: string; args: unknown } | { type: "tool_result"; callId: string; status: "ok" | "error"; content: ContentPart[] } | { type: "image"; mimeType: string; data: string };
...
export interface ToolCall { callId: string; name: string; args: unknown }
export interface JSONSchema { type?: string; description?: string; properties?: Record<string, JSONSchema>; required?: string[]; items?: JSONSchema; enum?: unknown[]; [key: string]: unknown }
export interface AgentTool<A = unknown, D = unknown> { name: string; description: string; schema: ZodType<A>; executionMode?: "parallel" | "sequential"; risk?: Risk; requiresContext?: boolean; execute(args: A, ctx?: ToolContext): Promise<AgentToolResult<D>> }
export interface AgentToolResult<D = unknown> { status: "ok" | "error"; content: ContentPart[]; details: D; usage?: Usage; terminate?: boolean }
...
export interface ToolContext { fs: FileSystem; exec(cmd: string, opts?: ExecOptions): Promise<Result<ExecOutput, ExecError>>; signal: AbortSignal; cwd: string }
...
export interface GrepOptions { path?: string; glob?: string; maxResults?: number }
export interface GrepMatch { file: string; line: number; text: string }

export interface TextFile { text: string; totalLines: number; truncated: boolean }
export interface FileInfo { path: string; kind: "file" | "dir" | "symlink"; size: number; mtime: number }
export interface DirEntry { name: string; kind: "file" | "dir" | "symlink" }

export interface ExecOptions { args?: string[]; cwd?: string; env?: Record<string, string>; timeoutMs?: number; onOutput?: (chunk: OutputChunk) => void; signal?: AbortSignal }
export interface ExecOutput { exitCode: number; stdout: string; stderr: string; truncated: boolean; durationMs: number }
export type OutputChunk = { stream: "stdout" | "stderr"; text: string };

export type FsErrorCode = "NOT_FOUND" | "PERMISSION" | "IS_DIR" | "NOT_DIR" | "EXISTS" | "OUT_OF_WORKSPACE" | "TOO_LARGE" | "IO";
...
export interface ApprovalPolicy { default: ApprovalMode; byRisk?: Partial<Record<Risk, ApprovalMode>>; byTool?: Record<string, ApprovalMode> }

// §7 三条队列 —— 消息缓冲，不是调度器
export type QueueName = "steering" | "followUp" | "nextRun";

// §9.1 Plan State（TODO）—— 唯一一份不受压缩影响的进度状态
export interface Todo { id: string; text: string; status: "pending" | "in_progress" | "completed" | "blocked"; note?: string }
export interface TodoState { items: Todo[]; updatedAt: number }
...
// §2 对外 API 面
export interface PromptAsset { name: string; content: string }
