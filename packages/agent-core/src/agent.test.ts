...
function toolEvents(calls: Array<{ name: string; args?: unknown; callId?: string }>, usage?: Usage): ModelEvent[] {
  ...
  calls.forEach((call, index) => {
    ...
    events.push({
      ...
      block: {
        type: "tool_call",
        callId: call.callId ?? `${call.name}-call-${index}`,
        name: call.name,
        args: call.args ?? {},
      },
    });
...
function fakeCtx(signal: AbortSignal): ToolContext {
  const fail = async () => ({
    ok: false as const,
    error: { code: "IO" as const, message: "not available in unit tests" },
  });
  return {
    fs: {
      read: fail,
...
function toolResultBlocks(
  entries: Awaited<ReturnType<SessionStorage["loadEntries"]>>,
): Array<Extract<Block, { type: "tool_result" }> & { role: string }> {
...
describe("装配校验（§1.1）", () => {
  it("Chat 模式接受自带执行环境的只读工具并能完成一次对话", async () => {
    ...
    expect(() => setup(empty, { chat: true, tools: [localTestTool("writer", { risk: "write" })] })).toThrow(
      /Chat 模式/,
    );
...
describe("结构化任务结果", () => {
  it("submit_result 终止 run 并通过 RunResult.output 返回成功结果", async () => {
    ...
    const { stream, requests } = scripted([
      toolEvents([{ name: "submit_result", args: output }]),
      textEvents("unused"),
    ]);
  ...
  it("submit_result 透传结构化失败，运行本身仍正常终止", async () => {
    const output = {
      ok: false as const,
      summary: "blocked",
      error: { code: "missing_input", message: "input is missing", retryable: false },
    };
...
describe("主流程", () => {
  ...
  it("tool 显式 error status 贯穿 Block、Record 与事件", async () => {
    ...
    const { agent, storage } = setup(stream, {
      tools: [localTestTool("failed", { risk: "read", status: "error", details: { reason: "nope" } })],
    });
    const events = recordEvents(agent);
    await agent.prompt("go");
    expect(toolResultBlocks(await storage.loadEntries(agent.sessionId))[0]!.status).toBe("error");
    expect(
      (await storage.loadRecords(agent.sessionId)).some(
        (record) => record.kind === "tool-finished" && record.status === "error",
      ),
    ).toBe(true);
...
describe("tool batch", () => {
  it("20 个 call 并发上限 8：在途不超过 8，结果按原始顺序回填", async () => {
    ...
    const entries = await storage.loadEntries(agent.sessionId);
    const resultMessage = entries.find(
      (item) =>
        item.kind === "message" &&
        item.message.role === "user" &&
        item.message.blocks.some((block) => block.type === "tool_result"),
    );
  ...
  it("sequential tool 独占执行：不与其他 task 重叠", async () => {
    ...
    const seq = localTestTool("seq", {
      ...
      onExecute: () => {
        inFlight += 1;
        seqMaxSeen = Math.max(seqMaxSeen, inFlight);
      },
      onDone: () => {
        inFlight -= 1;
      },
  ...
  it("terminate：全部结果置 terminate 才提前结束", async () => {
    ...
    const agent1 = setup(all.stream, {
      tools: [
        localTestTool("stop1", { risk: "read", terminate: true }),
        localTestTool("stop2", { risk: "read", terminate: true }),
      ],
    }).agent;
...
describe("队列", () => {
  it("steering 在 tool batch 完成后（排空点 A）注入当前 run", async () => {
    ...
    const steerIndex = entries.findIndex(
      (item) => item.kind === "message" && textOf(item.message.blocks) === "change direction",
    );
    const toolResultIndex = entries.findIndex(
      (item) => item.kind === "message" && item.message.blocks.some((block) => block.type === "tool_result"),
    );
    const finalIndex = entries.findIndex(
      (item) => item.kind === "message" && textOf(item.message.blocks) === "after steer",
    );
    ...
    expect(
      records.some(
        (item) => item.kind === "queue-enqueued" && item.queue === "steering" && item.message === "change direction",
      ),
    ).toBe(true);
  });
  ...
  it("followUp 在模型不再要工具时（排空点 B）注入并续跑", async () => {
    ...
    const entries = await storage.loadEntries(agent.sessionId);
    const followIndex = entries.findIndex(
      (item) => item.kind === "message" && textOf(item.message.blocks) === "keep going",
    );
    const firstAnswerIndex = entries.findIndex(
      (item) => item.kind === "message" && textOf(item.message.blocks) === "first answer",
    );
  ...
  it("nextRun 在当前 run 结束后触发一个独立 run", async () => {
    ...
    const records = await storage.loadRecords(agent.sessionId);
    const runStarted = records.filter(
      (item): item is Extract<SessionRecord, { kind: "run-started" }> => item.kind === "run-started",
    );
...
describe("TODO", () => {
  it("todo_write 成功 → TodoState 更新：todo-updated Record + todo.updated 事件", async () => {
    ...
    const { stream } = scripted([toolEvents([{ name: "todo_write", args: { items } }]), textEvents("ok")]);
    const { agent, storage } = setup(stream, {
      tools: [localTestTool("todo_write", { risk: "none", details: { items } })],
    });
    const events = recordEvents(agent);
    await agent.prompt("plan");

    const todoRecords = (await storage.loadRecords(agent.sessionId)).filter(
      (item): item is Extract<SessionRecord, { kind: "todo-updated" }> => item.kind === "todo-updated",
    );
...
describe("decision", () => {
  ...
  it("allow_always 写入 session allowlist：同名后续调用不再询问", async () => {
    ...
    const { agent } = setup(stream, {
      ...
      decide: async () => {
        decideCount += 1;
        return { kind: "approval", decision: "allow_always" };
      },
    });
    const result = await agent.prompt("go");
    expect(result.stopReason).toBe("done");
  ...
  it("abort 打断等待中的 decision → run aborted，decision 收敛", async () => {
    ...
    const resolved = (await storage.loadRecords(agent.sessionId)).find(
      (item): item is Extract<SessionRecord, { kind: "decision-resolved" }> => item.kind === "decision-resolved",
    );
...
describe("hooks", () => {
  ...
  it("prepareNextTurn 变更 model/thinking/activeTools → 落 Entry 且下一请求生效", async () => {
    ...
    const records = await storage.loadRecords(agent.sessionId);
    const turn1 = records.find(
      (item): item is Extract<SessionRecord, { kind: "turn-started" }> =>
        item.kind === "turn-started" && item.turn === 1,
    );
  ...
  it("spawn_agent：子 agent 独立 session，文本摘要/转录/usage 回到父，工具集受 depth 与参数约束", async () => {
    ...
    const spawnEnd = events.find(
      (event): event is Extract<AgentEvent, { type: "tool.end" }> =>
        event.type === "tool.end" && event.callId === "spawn-1",
    );
    ...
    const childRequest = requests.find((request) =>
      request.messages.some((message) => textOf(message.blocks).includes("child task")),
    );
...
describe("fork 与 resume", () => {
  it("fork：新分支独立生长，原分支不受影响", async () => {
    ...
    expect(
      textOf((parentBranch[1] as Extract<(typeof parentBranch)[number], { kind: "message" }>).message.blocks),
    ).toBe("A");
  ...
  async function interruptedFixture(
    storage: SessionStorage,
    sessionId: string,
    opts: { decision?: boolean; todos?: boolean } = {},
  ) {
    const user = entry({
      kind: "message",
      message: { id: "m-1", role: "user", createdAt: 1, blocks: [{ type: "text", text: "fix the bug" }] },
    });
    const assistant = entry({
      ...
      message: {
        ...
        role: "assistant",
        createdAt: 2,
        blocks: [{ type: "tool_call", callId: "c1", name: "ping", args: {} }],
      },
    });
    ...
    if (opts.todos) {
      await storage.appendRecord(
        sessionId,
        record("run-1", { kind: "todo-updated", items: todoList(["in_progress", "pending", "pending"]) }),
      );
    }
  }
