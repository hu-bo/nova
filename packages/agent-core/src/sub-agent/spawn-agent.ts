  ...
  return {
    run<T>(work: () => Promise<T>): Promise<T> {
      ...
      return new Promise<T>((resolve, reject) => {
        ...
        if (active.exit === null) {
          active.exit = (async () => {
            for await (const _ of active.flow.run()) {
              /* 只驱动调度 */
            }
          })();
          void active.exit.then(() => {
            if (current !== null && current.flow === active.flow) current = null;
          });
        }
        void active.exit.then(() => {
...
export function spawnAgentTool(
  ...
  return {
    name: "spawn_agent",
    description:
      "派生一个子 agent 独立完成子任务并等待其结果。子任务必须自包含：子 agent 只能看到你给它的任务描述。最多 4 个并发，超出排队。",
    schema: spawnInput,
    risk: "none",
