...
it.skipIf(!live)("真实模型自主完成复杂 coding 修复闭环", async () => {
  ...
  const harness = await createTestRuntime({
    ...
    stream: model.stream,
    maxTurns: 20,
    decide: async request => request.kind === "approval"
      ? { kind: "approval", decision: "allow" }
      : { kind: "question", answers: ["只修改 src 下的实现，不修改测试"] },
  });

  try {
    await writeFile(join(harness.workspace!, "package.json"), JSON.stringify({ type: "module", scripts: { test: "node test.js" } }, null, 2));
    const { mkdir } = await import("node:fs/promises");

    await mkdir(join(harness.workspace!, "src"));
    await writeFile(join(harness.workspace!, "src", "cart.js"), [
    ...
    expect(todoUpdates.at(-1)?.kind === "todo-updated" && todoUpdates.at(-1)?.items.every(item => item.status === "completed")).toBe(true);
  } finally {
