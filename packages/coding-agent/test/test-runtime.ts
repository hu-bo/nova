...
export async function startRunner(opts: { workspace?: string } = {}): Promise<RunnerHandle> {
  ...
  await sdk.listen();
  const binary =
    process.env.NOVA_RUNNER_BIN ??
    join(repoRoot, "target", "debug", process.platform === "win32" ? "nova-runner.exe" : "nova-runner");
  const child = spawn(binary, ["--server", sdk.endpoint, "--token", testToken, "--workspace", workspace], {
...
export function toolTurn(
  ...
  for (const [index, call] of calls.entries()) {
    ...
    events.push({
      type: "block.end",
      index,
      block: { type: "tool_call", callId: `c${callCounter}`, name: call.name, args: call.args },
    });
  }
...
export function scripted(turns: ModelEvent[][]): StreamFn {
  ...
  return async function* (_request, signal) {
    if (signal.aborted) {
      yield { type: "finish", stopReason: "aborted" };
      return;
    }
...
export async function createTestRuntime(opts: TestRuntimeOptions): Promise<TestRuntime> {
  ...
  const decide: Decide = async (request, signal) => {
