...
it("maps Anthropic text, usage, finish, and tool results", async () => {
  ...
  const events = await collect(anthropicStream({ provider: "anthropic", model: "claude", apiKey: "key" }, { max: 0 })(request, new AbortController().signal));
...
it("classifies only explicit context overflow as a typed failure", async () => {
  vi.stubGlobal("fetch", vi.fn(async () => new Response('{"error":{"message":"prompt is too long"}}', { status: 400 })));
  const events = await collect(anthropicStream({ provider: "anthropic", model: "claude", apiKey: "key" }, { max: 0 })(
    { system: "", messages: [], tools: [] },
    new AbortController().signal,
  ));
  expect(events).toEqual([{ type: "finish", stopReason: "error", errorMessage: expect.stringContaining("context overflow"), errorCode: "context_overflow" }]);
});
...
it("uses MiniMax Anthropic thinking control and preserves its signed thinking block", async () => {
  ...
  const events = await collect(anthropicStream({
    provider: "gateway",
    protocol: "anthropic",
    reasoningFormat: "minimax",
    model: "minimax-m3",
    apiKey: "key",
  }, { max: 0 })({ system: "", thinking: "high", tools: [], messages: [] }, new AbortController().signal));

  expect(sent.thinking).toEqual({ type: "adaptive" });
  expect(events).toContainEqual({ type: "block.end", index: 0, block: { type: "thinking", text: "plan", signature: "signed" } });
});
