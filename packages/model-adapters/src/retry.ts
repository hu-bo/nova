  ...
  const fallback =
    body.trim().length === 0
      ...
      : contentType.includes("html") || /^\s*</.test(body)
        ...
        : parsedJson
          ? "unrecognized JSON error response"
          : `unrecognized ${contentType || "non-JSON"} error response`;
  ...
  async function* run(): AsyncIterable<ModelEvent> {
    ...
    for (let retries = 0; ; retries += 1) {
      ...
      try {
        ...
        if (produced || retries >= max) {
          yield { type: "finish", stopReason: "error", errorMessage: failure };
          return;
        }
      } catch (error) {
        if (signal.aborted) {
        ...
        if (error instanceof ProviderError && error.retryAfterMs !== undefined) {
          ...
          if (!(await sleep(Math.min(error.retryAfterMs, maxMs), signal))) {
            yield { type: "finish", stopReason: "aborted" };
            return;
          }
          continue;
      ...
      if (!(await sleep(Math.random() * Math.min(maxMs, baseMs * 2 ** retries), signal))) {
        yield { type: "finish", stopReason: "aborted" };
        return;
      }
    }
