...
export const skillConnectionSchema = z
  .object({
    id,
    kind: z.literal("http"),
    baseUrl: z.url(),
  ...
  .superRefine((skill, context) => {
    ...
    for (const resource of skill.resources) {
      if (!safeRelativePath(resource.path)) {
        context.addIssue({
          code: "custom",
          path: ["resources", skill.resources.indexOf(resource), "path"],
          message: "must be a safe relative path",
        });
      }
      if (
        resource.content.kind === "inline" &&
        resource.content.encoding === "base64" &&
        !validBase64(resource.content.data)
      ) {
        context.addIssue({
          code: "custom",
          path: ["resources", skill.resources.indexOf(resource), "content", "data"],
    ...
    for (const connection of skill.connections) {
      ...
      if (url.protocol !== "https:" && url.hostname !== "localhost" && url.hostname !== "127.0.0.1") {
        context.addIssue({
          code: "custom",
          path: ["connections", skill.connections.indexOf(connection), "baseUrl"],
