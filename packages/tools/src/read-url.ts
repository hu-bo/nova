...
export const readUrl: Tool<z.output<typeof schema>> = {
  name: "read_url",
  description: "Read an uploaded URL. Supports text, images, PDF, DOCX, XLSX and CSV; private network addresses are rejected.",
  schema,
  risk: "read",
  ...
  async execute({ url }, ctx) {
    try {
      ...
      if (contentType.startsWith("image/")) {
        return { status: "ok", content: [{ type: "image", mimeType: contentType, data: Buffer.from(bytes).toString("base64") }], details: { url, contentType, size: bytes.byteLength } };
      }
      ...
      return { status: "ok", content: text(truncated ? `${extracted.slice(0, MAX_TEXT)}\n… content truncated` : extracted), details: { url, contentType, size: bytes.byteLength, truncated } };
    } catch (error) {
...
async function extractText(bytes: Uint8Array, contentType: string, pathname: string): Promise<string> {
  ...
  if (contentType.includes("wordprocessingml") || extension === "docx") return (await mammoth.extractRawText({ buffer })).value;
  if (contentType.includes("spreadsheetml") || contentType.includes("excel") || extension === "xlsx" || extension === "xls") {
    const workbook = XLSX.read(buffer);
    return workbook.SheetNames.map(name => `# ${name}\n${XLSX.utils.sheet_to_csv(workbook.Sheets[name]!)}`).join("\n\n");
  }
  if (contentType.startsWith("text/") || contentType.includes("json") || ["md", "txt", "csv", "tsv", "json", "xml", "yaml", "yml"].includes(extension ?? "")) return buffer.toString("utf8");
