...
describe("chat-ui", () => {
  it("renders markdown and structured code", () => {
    ...
    const code = renderToStaticMarkup(<BlockView block={{ type: "code", language: "ts", code: "const done = true", path: "src/a.ts", startLine: 4 }} />);
  ...
  it("pairs tool calls and results while hiding todo_write", () => {
    const messages: ChatMessage[] = [{
      ...
      blocks: [
        ...
        { type: "tool_result", callId: "call-2", status: "ok", blocks: [{ type: "todo", items: [{ id: "1", text: "ship", status: "pending" }] }] },
      ],
    }];
  ...
  it("renders accessible decisions and read-only todos", () => {
    const decision = renderToStaticMarkup(<DecisionPrompt request={{ kind: "approval", decisionId: "d1", toolName: "bash", args: { command: "rm build/output" }, risk: "exec" }} onResolve={() => undefined} />);
  ...
  it("supports instance renderer overrides", () => {
    const html = renderToStaticMarkup(<BlockView block={{ type: "text", text: "raw" }} renderers={{ text: ({ block }) => <mark>{block.type}</mark> }} />);
  ...
  it("renders file, model, and reasoning controls in the composer", () => {
    const html = renderToStaticMarkup(<Composer
      ...
      reasoningEfforts={[{ value: "low", label: "低" }, { value: "high", label: "高" }]}
