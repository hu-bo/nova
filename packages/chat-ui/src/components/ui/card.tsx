...
export function Card({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col rounded-xl bg-white text-slate-950 shadow-sm ring-1 ring-slate-200/80 dark:bg-slate-950 dark:text-slate-50 dark:ring-slate-800",
        className,
      )}
      {...props}
    />
...
export function CardTitle({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("font-semibold leading-none text-slate-900 dark:text-slate-100", className)}
      {...props}
    />
  );
}
