...
const statusView = {
  ...
  in_progress: {
    label: "进行中",
    Icon: LoaderCircle,
    iconClass: "animate-spin text-indigo-500 motion-reduce:animate-none",
  },
      ...
      {items.map((item) => {
        ...
        return (
          <li
            ...
            className="grid grid-cols-[1.25rem_minmax(0,1fr)] items-start gap-2 rounded-lg px-2 py-1.5"
          >
            <Icon className={`mt-0.5 size-4 ${iconClass}`} aria-hidden="true" />
            <span className="min-w-0">
              <span
                className={`block text-sm leading-5 ${item.status === "completed" ? "text-slate-400 line-through" : "text-slate-700 dark:text-slate-300"}`}
              >
                {item.text}
              </span>
