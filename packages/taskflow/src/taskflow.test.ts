...
describe("taskflow", () => {
  it("runs a dependency chain in order and passes results via ctx.results", async () => {
    ...
    flow.addTask({
      ...
      deps: ["a"],
      run: async (ctx) => {
        expect(ctx.results.get("a")).toBe(42);
        return "b-done";
      },
    });
    ...
    expect(last?.type === "flow.finished" && last.counts).toEqual({
      succeeded: 2,
      failed: 0,
      timedOut: 0,
      cancelled: 0,
  ...
  it("times out a task and consumes retry budget on timeouts", async () => {
    ...
    flow.addTask({
      ...
      run: async (ctx) => {
        await abortableSleep(ctx.signal, 5_000);
      },
    });
    const events = await drain(flow);
    expect(shape(events)).toEqual([
    ...
    flow.addTask({
      ...
      run: async (ctx) => {
        ctx.addTask({
          ...
          deps: ["a"],
          run: async (inner) => {
            expect(inner.results.get("a")).toBe("a-done");
            return "b-done";
          },
        });
        return "a-done";
  ...
  it("an empty flow finishes immediately with zero counts", async () => {
    ...
    expect(events[0]).toEqual({
      type: "flow.finished",
      counts: { succeeded: 0, failed: 0, timedOut: 0, cancelled: 0, skipped: 0 },
    });
  });
