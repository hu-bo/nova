...
const ProvidersRoute = lazy(() => import("./routes/providers.js").then(module => ({ default: module.ProvidersRoute })));
...
const queryClient = new QueryClient({ defaultOptions: { queries: { retry: 1, staleTime: 10_000, refetchOnWindowFocus: true }, mutations: { retry: false } } });
...
export function App() {
  ...
  return <AuthProvider><QueryClientProvider client={queryClient}><BrowserRouter><Routes><Route path="/callback" element={<LazyPage><CallbackRoute /></LazyPage>} /><Route element={<RequireAuth><LazyPage><AppShell /></LazyPage></RequireAuth>}><Route index element={<Navigate to="/providers" replace />} /><Route path="/providers" element={<LazyPage><ProvidersRoute /></LazyPage>} /><Route path="/models" element={<LazyPage><ModelsRoute /></LazyPage>} />
    <Route path="/usage" element={<LazyPage><UsageRoute /></LazyPage>} /><Route path="/quotas" element={<LazyPage><QuotasRoute /></LazyPage>} /></Route><Route path="*" element={<Navigate to="/providers" replace />} /></Routes></BrowserRouter></QueryClientProvider></AuthProvider>;
}
