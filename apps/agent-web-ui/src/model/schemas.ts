...
export const modelProfileSchema = z.object({
  ...
  endpoint: z.url("请输入有效的 HTTPS 地址").refine(value => new URL(value).protocol === "https:", "必须使用 HTTPS"),
  ...
  path: ["maxOutput"],
  message: "不能大于上下文窗口",
}).refine(value => value.reasoningFormat === "none" ? value.thinkingLevels.length === 0 : value.thinkingLevels.length > 0, {
  path: ["thinkingLevels"],
