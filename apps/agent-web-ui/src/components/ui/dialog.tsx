...
export function Dialog({ open, title, description, children, onClose, size = "md", placement = "center" }: DialogProps) {
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={next => { if (!next) onClose(); }}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Viewport className={cn("fixed inset-0 z-50 flex overflow-y-auto", placement === "left" ? "items-stretch justify-start p-0" : "items-center justify-center p-4")}>
          <DialogPrimitive.Popup
            ref={popupRef}
            initialFocus={() => popupRef.current?.querySelector<HTMLElement>("[data-initial-focus], input:not([disabled]), select:not([disabled]), button:not([disabled])") ?? null}
          ...
          >
            <header className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
              ...
              <DialogPrimitive.Close aria-label="关闭" className="rounded-lg p-2 text-slate-400 outline-none transition hover:bg-slate-100 hover:text-slate-700 focus-visible:ring-2 focus-visible:ring-indigo-500">
                <X className="size-5" aria-hidden="true" />
              </DialogPrimitive.Close>
            </header>
            <div className={cn("overflow-y-auto px-6 py-5", placement === "left" ? "min-h-0 flex-1" : "max-h-[min(80vh,900px)]")}>{children}</div>
          </DialogPrimitive.Popup>
