...
function updateMessage(messages: ChatMessage[], messageId: string, update: (message: ChatMessage) => ChatMessage): ChatMessage[] {
  let changed = false;
...
function appendBlockDelta(messages: ChatMessage[], messageId: string, index: number, delta: string): ChatMessage[] {
  return updateMessage(messages, messageId, message => {
    ...
    const blocks = [...message.blocks];
    blocks[index] = current.type === "code"
      ? { ...current, code: current.code + delta }
      : { ...current, text: current.text + delta };
    return { ...message, blocks };
  });
}

function appendToolOutput(messages: ChatMessage[], callId: string, stream: "stdout" | "stderr", text: string): ChatMessage[] {
  return messages.map(message => {
    ...
    if (resultIndex >= 0) {
      ...
      const outputBlocks: Block[] = first?.type === "code"
