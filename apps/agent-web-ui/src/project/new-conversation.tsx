...
export function NewConversationDialog({ open, onClose, initialProjectId, initialMode }: NewConversationDialogProps) {
  ...
  return (
    <Dialog
      open={open}
      onClose={onClose}
      title={initialProjectId ? "新建 Project 会话" : initialMode === "chat" ? "新建普通会话" : "开始一个新会话"}
      description={
        initialProjectId
          ? "新会话将使用当前 Project 的 Runner 与 workspace。"
          : initialMode === "chat"
            ? "用于讨论、分析和多步规划。"
            : "模式创建后不可更改；需要访问代码时请选择 Project。"
      }
      size="lg"
    >
      <form onSubmit={submit} className="space-y-6">
        {!initialProjectId && !initialMode && (
          <div className="grid gap-3 sm:grid-cols-2" role="radiogroup" aria-label="会话模式">
            <ModeOption
              active={mode === "chat"}
            ...
            <Select {...form.register("modelProfileId")}>
              ...
              {models.profiles.map((profile) => (
                <option key={profile.id} value={profile.id}>
                  {profile.model} ({profile.providerName})
                </option>
              ))}
            </Select>
          </FieldLabel>
        ...
        {mode === "chat" && (
          <FieldLabel label="Runner" hint="Chat 保留设备归属，但不会使用文件工具" error={fieldErrors.runnerId?.message}>
            <RunnerSelect
              ...
              onChange={(value) => form.setValue("runnerId", value, { shouldValidate: true })}
            />
          </FieldLabel>
        )}
              ...
              <Select {...form.register("projectId")}>
                ...
                {projects.data?.map((project) => (
                  <option key={project.id} value={project.id}>
                    {project.name}
                    {project.workspace ? ` · ${project.workspace}` : "（待绑定）"}
                  </option>
                ))}
        ...
        <div className="flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
          ...
          <Button
            ...
            icon={create.isPending ? undefined : <Plus className="size-4" aria-hidden="true" />}
          >
...
function ModeOption({
  ...
  return (
    ...
    >
      <span
        className={`mb-3 grid size-9 place-items-center rounded-lg ${active ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-600"}`}
      >
        {icon}
      </span>
