...
}).refine(query => !query.from || !query.to || query.from <= query.to, { message: "from must be before to", path: ["to"] });
...
const UsageReportSchema = z.object({
  ...
  totals: z.object({ requests: z.number().int().nonnegative(), input: z.number().int().nonnegative(), output: z.number().int().nonnegative(), cacheRead: z.number().int().nonnegative(), cost: z.string() }),
...
export function usageRoutes(app: FastifyInstance, store: ModelConfigStore): void {
  ...
  app.withTypeProvider<ZodTypeProvider>().get("/usage", {
    schema: { operationId: "getUsage", tags: ["model-config"], security: [{ bearerAuth: [] }], querystring: UsageQuerySchema, response: { 200: UsageReportSchema, 400: ErrorSchema, 401: ErrorSchema, 403: ErrorSchema } },
