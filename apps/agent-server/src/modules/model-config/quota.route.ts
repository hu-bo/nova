...
const UpdateQuotaSchema = z.strictObject({
  ...
  monthlyCost: z.string().regex(/^\d+(\.\d{1,8})?$/).nullable(),
});
const QuotaSchema = UpdateQuotaSchema.extend({ apiKeyId: z.uuid(), keyName: z.string(), keyPrefix: z.string(), updatedAt: z.iso.datetime() });
...
export function quotaRoutes(app: FastifyInstance, store: ModelConfigStore): void {
  ...
  server.get("/quotas", {
    schema: { operationId: "listQuotas", tags: ["model-config"], security: [{ bearerAuth: [] }], response: { 200: z.array(QuotaSchema), 401: ErrorSchema, 403: ErrorSchema } },
    preHandler: requireAdmin,
  }, async () => (await store.listQuotas()).map(quotaView));
  server.patch("/quotas/:apiKeyId", {
    schema: { operationId: "updateQuota", tags: ["model-config"], security: [{ bearerAuth: [] }], params: ApiKeyParams, body: UpdateQuotaSchema, response: { 200: QuotaSchema, 400: ErrorSchema, 401: ErrorSchema, 403: ErrorSchema, 404: ErrorSchema } },
    preHandler: requireAdmin,
