...
const CreateModelSchema = z
  .strictObject({
    ...
    inputModalities: z
      ...
      .min(1)
      .refine((items) => new Set(items).size === items.length && items.includes("text")),
    enabled: z.boolean(),
    priceIn: MoneySchema,
  ...
  .superRefine((model, context) => {
    if (model.maxOutput > model.contextWindow)
      context.addIssue({ code: "custom", path: ["maxOutput"], message: "maxOutput cannot exceed contextWindow" });
    if (model.reasoningFormat === "none" && model.thinkingLevels.length)
      context.addIssue({
        code: "custom",
        path: ["thinkingLevels"],
        message: "thinkingLevels must be empty when reasoningFormat is none",
      });
    if (model.reasoningFormat !== "none" && !model.thinkingLevels.length)
      context.addIssue({
        code: "custom",
        path: ["thinkingLevels"],
        message: "thinkingLevels is required for reasoning-enabled models",
      });
  });
  ...
  server.get(
    "/models",
    {
      schema: {
        ...
        tags: ["model-config"],
        security: [{ bearerAuth: [] }],
        response: { 200: z.array(ModelSchema), 401: ErrorSchema, 403: ErrorSchema },
      },
    },
