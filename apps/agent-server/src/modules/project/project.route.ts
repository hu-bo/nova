...
export function projectRoutes(app: FastifyInstance, store: AgentStore, runners: RunnerRegistry): void {
  ...
  server.post(
    ...
    {
      schema: {
        operationId: "bindProjectWorkspace",
        tags: ["projects"],
        security: [{ bearerAuth: [] }],
        params: IdParams,
        body: BindProjectWorkspaceSchema,
        response: {
          200: ProjectSchema,
          400: ApiErrorSchema,
          401: ApiErrorSchema,
  ...
  server.delete(
    ...
    async (request, reply) => {
      if (request.headers["x-confirm-delete"] !== request.params.id)
        throw invalidInput("Project deletion must be confirmed with its id");
