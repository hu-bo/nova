...
export function runnerRoutes(app: FastifyInstance, store: AgentStore, registry: RunnerRegistry, endpoint: string): void {
  const server = app.withTypeProvider<ZodTypeProvider>();
  const runners = createRunnerService(store, registry);

  server.get("/runner-connection", {
    schema: {
      operationId: "getRunnerConnectionInfo", tags: ["runners"], security: [{ bearerAuth: [] }],
      response: { 200: RunnerConnectionInfoSchema, 401: ApiErrorSchema },
    },
  }, () => ({ endpoint }));

  server.get("/runner-tokens", {
    schema: {
      operationId: "listRunnerTokens", tags: ["runners"], security: [{ bearerAuth: [] }],
      response: { 200: z.array(RunnerTokenSchema), 401: ApiErrorSchema },
    },
  }, request => runners.listTokens(request.userId));

  server.post("/runner-tokens", {
    schema: {
      operationId: "createRunnerToken", tags: ["runners"], security: [{ bearerAuth: [] }],
  ...
  server.delete("/runner-tokens/:id", {
    schema: {
      operationId: "deleteRunnerToken", tags: ["runners"], security: [{ bearerAuth: [] }], params: TokenParams,
  ...
  server.get("/runners", {
    schema: {
      operationId: "listRunners", tags: ["runners"], security: [{ bearerAuth: [] }], querystring: RunnerQuerySchema,
  ...
  server.get("/runners/directories", {
    schema: {
      operationId: "listRunnerDirectories", tags: ["runners"], security: [{ bearerAuth: [] }], querystring: RunnerDirectoryQuerySchema,
  ...
  server.delete("/runners/:id", {
    schema: {
      operationId: "deleteRunner", tags: ["runners"], security: [{ bearerAuth: [] }], params: RunnerParams,
      response: { 204: z.null(), 401: ApiErrorSchema, 404: ApiErrorSchema, 409: ApiErrorSchema },
    },
  }, async (request, reply) => {
  ...
  server.get("/runners/events", {
    schema: {
      operationId: "subscribeRunnerEvents", tags: ["runners"], security: [{ bearerAuth: [] }],
